...
Native Android apps (Kotlin or Java) use the TracePath Android SDK, `com.tracepathhq:tracepath`, published on Maven Central. It reports **errors and crashes only**: there is no session or video replay (unlike Flutter). It speaks the same `/api/report` wire format as the iOS, Flutter, and JS SDKs, so the backend ingests it with no server changes.
...
```kotlin
dependencies {
    implementation("com.tracepathhq:tracepath:1.0.1")
}
```
...
Call `TracePath.init` from `Application.onCreate()`. This wraps the whole process, so every uncaught exception on every thread is captured automatically:

```kotlin
import android.app.Application
import com.tracepathhq.tracepath.TracePath
import com.tracepathhq.tracepath.TracePathOptions
...
class MyApp : Application() {
    override fun onCreate() {
        ...
        TracePath.init(
            ...
            options = TracePathOptions(version = "1.0.0"),
        )
    }
}
```
...
try {
    ...
    TracePath.captureException(e)
}

TracePath.captureMessage("User completed onboarding")
TracePath.flush()   // force-send pending events
```
...
The last ~10 seconds of logs and actions ride along with each captured exception. `println` / `System.out` / `System.err` are mirrored automatically; `android.util.Log` lines are not, so install a Timber tree that calls `TracePathClient.instance?.recordLog(...)` if you need Logcat. Full option list (`sampleRate`, `debug`, channel toggles, on-disk persistence) is at https://docs.tracepath.dev/client/android.

## Symbolication: upload R8 `mapping.txt` with the Gradle plugin

A release build with `minifyEnabled true` runs R8, which renames classes and methods and rewrites the line-number table, so production crash traces arrive obfuscated and stay unreadable until TracePath retraces them server-side against the build's `mapping.txt` (the Android equivalent of a JavaScript source map). Set this up whenever the app ships a minified release.
...
pluginManagement {
  repositories {
    gradlePluginPortal()
    mavenCentral()   // resolves com.tracepathhq.symbols
  }
}
```
...
plugins {
  ...
  id("com.tracepathhq.symbols") version "1.0.1"
}
...
tracepath {
  ...
  uploadToken = System.getenv("TRACEPATH_UPLOAD_TOKEN") ?: ""
...
Order matters in the `plugins { }` block. Gradle applies entries top to bottom, and `com.tracepathhq.symbols` looks for the Android extension the moment it is applied, so it has to come **after** `com.android.application` (or `com.android.library`). Declared above it, the build fails at configuration time with `com.tracepathhq.symbols must be applied after an Android application/library plugin`. The app module already has the Android plugin line, so add the TracePath line under it rather than pasting a new block above it.

**Wire the UUID into the SDK.** The plugin embeds a ProGuard UUID into `BuildConfig.TRACEPATH_PROGUARD_UUID` and names the uploaded mapping `<uuid>.txt`. The UUID is derived from the module path, variant, and app version (`versionName` + `versionCode`), so **bump the version each release** or a new upload overwrites the previous one (or set `proguardUuid` explicitly). Pass the injected value to the SDK so each reported crash carries the matching UUID:
...
TracePath.init(
    ...
    options = TracePathOptions(
        version = "1.0.0",
        proguardUuid = BuildConfig.TRACEPATH_PROGUARD_UUID,
    ),
)
```
...
```bash
./gradlew assembleRelease uploadReleaseTracePathSymbols
```

With `autoUpload = true` the upload runs automatically after `assembleRelease`. Keep the upload token in a CI secret. The `System.getenv` wiring in the `tracepath { ... }` block above is what feeds it to the plugin (there is no built-in environment lookup):

```bash
TRACEPATH_UPLOAD_TOKEN=<upload-token> ./gradlew assembleRelease uploadReleaseTracePathSymbols
```
...
1. Add a button that throws (`throw RuntimeException("Test error from TracePath")`), tap it, and confirm the error appears on the Issues page within a few seconds with its full stack trace.
2. For a minified release build: trigger a crash, then after `uploadReleaseTracePathSymbols` has run for that build, confirm the stack trace on Issues shows real class, method, and `file:line` rather than obfuscated `a.a.a(SourceFile)` frames.

For the symbolicator internals (how a trace is recognized, the retrace, and the `.tw` cache), see https://docs.tracepath.dev/symbolicator/android.
