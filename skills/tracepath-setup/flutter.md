...
The TracePath Flutter SDK is the [`tracepath`](https://pub.dev/packages/tracepath) package on pub.dev. It supports iOS, Android, and macOS. It does NOT support Flutter web (see the bottom of this file).

## Setup

```bash
flutter pub add tracepath
```

Wrap the app in `TracePath.run()`:
...
```dart
MaterialApp(
  navigatorObservers: [TracePath.navigatorObserver],
  home: const HomePage(),
);
```
...
try {
  ...
  TracePathClient.instance?.captureException(e, st);
}

TracePathClient.instance?.captureMessage('User completed onboarding');
```
...
| `captureNavigation` | `bool` | `true` | Record transitions reported by `TracePath.navigatorObserver` |
...
Mask sensitive content with `TracePathPrivacyMask` (applies to recorded frames only):
...
TracePathPrivacyMask(
  mode: TracePathMaskMode.blur(ratio: 0.5),
  child: CreditCardWidget(),
)

TracePathPrivacyMask(
  mode: TracePathMaskMode.blank(color: Color(0xFF000000)),
...
- **Navigation actions**: push / pop / replace / remove, from `TracePath.navigatorObserver`.
- **Custom actions**:
  ```dart
  TracePath.recordAction(
    category: 'cart',
    name: 'add_item',
...
The `tracepath` package ships an uploader (`dart run tracepath:upload_symbols`) that finds the `.symbols` files, derives each one's architecture and debug ID, and posts them. Configure it once, then run it on every release.

**One-time setup.** Add an `tracepath:` block to the app's `pubspec.yaml`:

```yaml
tracepath:
  url: https://<instance>   # omit on TracePath Cloud
  # upload_token: ...        # prefer the env var below, especially in CI
```

The upload token resolves from `TRACEPATH_UPLOAD_TOKEN` (or `--token`) before the `upload_token` key, so keep it out of `pubspec.yaml` in CI.
...
TRACEPATH_UPLOAD_TOKEN=<upload-token> dart run tracepath:upload_symbols
```

The uploader auto-discovers `build/symbols` and uploads every architecture in one run, so no flags are required. Each value resolves from a CLI flag first (`--token`, `--url`, `--symbols-dir`), then an env var (`TRACEPATH_UPLOAD_TOKEN`, `TRACEPATH_URL`), then the `pubspec.yaml` block. Pass `--dry-run` to preview, but note that it still requires a resolved token and URL: without them it exits with `No upload token. Set TRACEPATH_UPLOAD_TOKEN, pass --token, or add tracepath.upload_token to pubspec.yaml`. Symbols are unique to each build, so upload on every release; a crash only resolves against the exact build it came from.
...
- name: Upload Flutter symbols
  run: dart run tracepath:upload_symbols
  env:
    TRACEPATH_URL: ${{ secrets.TRACEPATH_URL }}
    TRACEPATH_UPLOAD_TOKEN: ${{ secrets.TRACEPATH_UPLOAD_TOKEN }}
```
...
For network capture in Dart code on web (where `HttpOverrides.global` does not run), `TracePathHttpClient` is a drop-in `http.Client`, usable directly or passed to libraries that accept a custom client (Dio, Chopper):

```dart
final client = TracePathHttpClient();
final res = await client.get(Uri.parse('https://api.example.com/users'));
```

## Verify

Add a button that throws (`throw StateError('Test error from TracePath')`), tap it, and check the Issues page in the dashboard. With `screenCapture: true`, the recording appears alongside the stack trace. For an obfuscated release build, confirm the stack trace shows real frames rather than obfuscated offsets once the matching `.symbols` files have been uploaded.
