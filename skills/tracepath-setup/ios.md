...
| App | Path |
|---|---|
| **Native Swift** (SwiftUI or UIKit) | The TracePath iOS SDK (below). Recommended. Native crash and error capture, reports to `/api/report` like the other TracePath mobile SDKs. |
| **Anything else** (Objective-C only, a cross-platform stack with no TracePath mobile SDK, or a team already standardized on OpenTelemetry) | An OpenTelemetry distribution such as Honeycomb's, with its OTLP exporter repointed at TracePath. See "Non-Swift apps" at the bottom. |

The TracePath iOS SDK reports **errors and crashes only**. There is no session or video replay (unlike Flutter). It speaks the same `/api/report` wire format as the Android, Flutter, and JS SDKs, so the backend ingests it with no server changes.
...
Call `TracePath.start` as early as possible, in the `App` initializer (SwiftUI) or `application(_:didFinishLaunchingWithOptions:)` (UIKit). The connection string format is `<project-token>@https://<instance>/api/report`, identical to the other mobile SDKs.
...
Wire the token through a build setting or `Info.plist` value, not a committed literal. `TracePath.start` returns `nil` and no-ops on a malformed connection string, so a bad value fails quietly rather than crashing the host app.
...
do {
    ...
    TracePath.capture(error)            // Swift Error
}

TracePath.capture(error: nsError)       // NSError
TracePath.capture(message: "User completed onboarding")
```
...
```swift
TracePath.flush(timeout: 5) // seconds; nil = wait indefinitely
```
...
```bash
TRACEPATH_UPLOAD_TOKEN=<upload-token>
TRACEPATH_URL=https://<instance>        # base URL; the script appends /api/symbols/upload
```

To upload by hand (for example from CI after an `xcodebuild archive`), point it at the dSYM directory:

```bash
TRACEPATH_UPLOAD_TOKEN=<upload-token> TRACEPATH_URL=https://<instance> \
...
find "$DWARF_DSYM_FOLDER_PATH" -type f -path '*.dSYM/Contents/Resources/DWARF/*' \
  -exec curl -fsS -H "Authorization: Bearer $TRACEPATH_UPLOAD_TOKEN" \
    -F "files=@{}" "$TRACEPATH_URL/api/symbols/upload" \;
```
...
1. In a Debug build, tap a button that calls `TracePath.capture(SomeError())`, then `TracePath.flush(timeout: 5)`, and confirm it appears on the Issues page within a few seconds.
...
## Non-Swift apps: OpenTelemetry into TracePath

For an iOS/Apple app that is not a Swift app the SDK targets (Objective-C only, a cross-platform stack with no TracePath mobile SDK, or a codebase already standardized on OpenTelemetry), there is no native TracePath SDK. Use an OpenTelemetry distribution such as **Honeycomb's** and repoint its OTLP exporter at TracePath. This is the same OTLP/HTTP path the backend uses (see "Backend OTel Setup" in SKILL.md), so all the rules in `data-model.md` apply.

A Honeycomb-style distro is configured with an API key plus an endpoint. Override both so the data lands in TracePath instead of Honeycomb:

- **Endpoint**: `https://<instance>/api/otel` (the SDK appends `/v1/traces`, `/v1/metrics`, `/v1/logs`).
- **Auth header**: `Authorization: Bearer <project-token>`. TracePath authenticates on this Bearer header, not on Honeycomb's `x-honeycomb-team`, so set the `Authorization` header explicitly (Honeycomb's own API-key header is ignored by TracePath and harmless).
...
The method is `setAPIEndpoint`, with a capital `API`, matching `setAPIKey`. Swift is case-sensitive, so `setApiEndpoint` does not compile. Do not drop the line if it fails: without it the exporter falls back to Honeycomb's own default endpoint and the telemetry never reaches TracePath.
