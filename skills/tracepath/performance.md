...
Authoritative reference for the **Performance** flow. Approach the investigation as a senior software engineer with deep performance-debugging experience: do not guess at fixes, quantify first, localize the cost to a specific span, confirm the cause against the checklist below, and only then propose a change. Every suspect here is paired with the TracePath signal that confirms it, so the checklist is a lookup table from "what the waterfall shows" to "what is actually wrong".

All commands are read-only. Never archive or mutate anything during a performance investigation. For exact CLI syntax, the mandatory `--recorded-at` / `--started-at` timestamp rules, and dashboard-URL resolution, the parent `SKILL.md` is authoritative (over MCP: the `tracepath://knowledge/timestamps` and `tracepath://knowledge/url-resolution` resources).

## The investigation loop

1. **Quantify and localize.** Find the slow endpoint(s) and read the shape of the latency:
   ```bash
   tracepath endpoints list --since 24h --order-by p95 --page-size 10 --output json \
...
2. **Get a representative slow trace.** Drill into one slow request to see where the time goes. Capture an id and its `recordedAt` together, then:
   ```bash
   tracepath endpoints show <endpointId> --recorded-at <t>          # span waterfall for one request
   tracepath traces show <distributedTraceId> --recorded-at <t>     # cross-service timeline
   ```
...
4. **Separate code from saturation.** A slow endpoint can be a code bug or an overloaded host. Pull infrastructure and runtime metrics over the same window:
   ...
   tracepath metrics query --name mem.used --aggregation max --since 24h
   tracepath metrics query --name go.gc_pause --aggregation max --since 24h
   ```
...
```bash
tracepath endpoints slow "GET /api/reports/export" --output json   # {offsetMs, reason}
```
...
```bash
tracepath endpoints chart --metric-type p95 --since 7d --interval-minutes 60 --output json \
...
```bash
tracepath metrics query --name system.cpu.utilization --aggregation max --since 7d --interval-minutes 60 --output json \
...
tracepath endpoints list --search "GET /api/checkout" --from 2026-06-25T00:00:00Z --to 2026-06-25T12:00:00Z \
  --output json | jq '.data[0] | {p95, p99, count}'
tracepath endpoints list --search "GET /api/checkout" --from 2026-06-25T12:00:00Z --to 2026-06-26T00:00:00Z \
  --output json | jq '.data[0] | {p95, p99, count}'
```
...
Each row: the suspect, and how to confirm it from TracePath telemetry.

### Database and data access
| Suspect | How to confirm in TracePath |
|---|---|
...
- Host and runtime metrics live under `system.*` (TracePath OTel Agent) and the SDK runtime names (`go.gc_pause`, `mem.used`, `go.go_routines`). The server has no quantile aggregation for metric points: `p50/p95/p99` on `metrics query` silently return `avg`, so never present those as percentiles. Real latency percentiles come only from `endpoints list`, computed from raw request durations.
