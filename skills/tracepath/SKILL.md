---
name: tracepath
description: 'Operate a TracePath observability instance through the TracePath CLI: log in, query exceptions, logs, endpoints, and metrics, and debug production issues down to root cause. Use when the user invokes /tracepath with a subcommand, e.g. "/tracepath login", "/tracepath debug issue <hash|url|title>", "/tracepath what''s broken in prod", or whenever they want to investigate errors, crashes, slowness, or logs from an app monitored by TracePath.'
---
...
Drive a TracePath instance from the terminal with the `tracepath` CLI. The first word of the argument decides the flow:

| Invocation | Flow |
|---|---|
| `/tracepath login` | **Login**: install the CLI if missing, authenticate, select a project |
| `/tracepath debug <issue ref or bug description>` | **Debug**: resolve the issue and investigate to root cause |
| `/tracepath perf <endpoint or symptom>` | **Performance**: diagnose latency/slowness to root cause against a checklist of common bottlenecks |
| `/tracepath <anything else>` | **Query**: answer the observability question with CLI reads |
| Ask what they want: log in, debug an issue, or run a query |

> The CLI is under active development. If a flag documented here does not appear in `tracepath <command> --help`, trust the binary.
> If a `tracepath` MCP server is connected, prefer its tools over shelling out to the CLI: they wrap the same API with the same semantics, and this skill's knowledge is available as its resources. The server is this same binary (`tracepath mcp`).
...
- On exit code 4 (auth), do not run `tracepath login` yourself; switch to the Login flow and let the user enter credentials.
...
|---|---|---|
| `/issues/<hash>` and `/issues/<hash>/events` | Exception group (hash = 16 hex chars) | `tracepath exceptions show <hash>` |
| `/issues/<hash>/<occurrenceId>` (UUID) | One occurrence within the group | `tracepath exceptions occurrence <occurrenceId> --recorded-at <t>` where `t` is the URL's `?t=` param. Direct and fast; also returns the occurrence's `sessionId` and session recording. No URL? get `recordedAt` from `tracepath exceptions show <hash>` occurrences |
| `/endpoints/<endpoint>` | Endpoint group; the segment is the URL-encoded endpoint name (`GET%20%2Fapi%2Fusers%2F%3Aid` is `GET /api/users/:id`) | Decode it, then `tracepath endpoints list --search "<decoded name>"` (the group has no id; `endpoints show` is for one request — next row) |
| `/endpoints/<endpoint>/<endpointId>` | One request (transaction) of that endpoint | `tracepath endpoints show <endpointId> --recorded-at <t>` (`t` = the URL's `?t=` param). Returns the request, its span waterfall, and any linked exception/messages |
| `/tasks/<task>` | Background task group | No CLI for the group; for one run use the next row |
| `/tasks/<task>/<taskId>` | Single task run | `tracepath tasks show <taskId> --recorded-at <t>` (`t` = the URL's `?t=` param) |
| `/sessions/<sessionId>` | Session (the exceptions that fired during it; replay stays dashboard-only) | `tracepath sessions show <sessionId> --started-at <t>`. The URL has no `?t=`; use the session's start, the URL's `from=`, or a linked occurrence's `recordedAt` (it falls inside the window). Occurrences reference sessions via their `sessionId` |
| `/ai-traces/<traceName>` | AI trace group | No CLI for the group; for one trace use the next row |
| `/ai-traces/<traceName>/<traceId>` | Single AI trace | `tracepath ai-traces show <traceId> --recorded-at <t>` (`t` = the URL's `?t=` param); returns token/cost stats + the conversation |
| `/logs` | Logs page (its filters are not stored in the URL) | `tracepath logs query` with flags taken from the user's description |
...
1. **Recover it from an API.** For an occurrence whose hash you know (e.g. `/issues/<hash>/<occurrenceId>` pasted without `?t=`), run `tracepath exceptions show <hash>` and read that occurrence's `recordedAt` — the hash endpoint needs no timestamp. A group's `firstSeen`/`lastSeen` from `exceptions list` bound when its occurrences happened (`lastSeen` ≈ the most recent one).
...
```bash
tracepath exceptions occurrence <Exception ID> --recorded-at <Occurred at → RFC3339> --output json
...
tracepath endpoints list --search "<endpoint>" --since 1h --output json | jq '.data[0] | {p50, p95, p99, count, impact, impactReason}'
# 2. is this an accepted baseline? check whether an operator marked it slow
tracepath endpoints slow "<endpoint>" --output json    # {offsetMs, reason}; offsetMs 0 = not marked
# 3. find when it crossed the threshold, then a slow trace (see Flow: Performance)
tracepath endpoints chart --metric-type p95 --since 6h --interval-minutes 15 --output json
```
...
install -m 755 "$TMP/tracepath" ~/.local/bin/tracepath && rm -rf "$TMP"
```
...
Verify with `tracepath version`.

### 3. Authenticate

Login prompts for the password interactively, so ask the user to run it themselves (in Claude Code, suggest typing `! tracepath login --url https://<instance>` so the output lands in the session):

```bash
tracepath login --url https://<tracepath-instance>
```
...
```bash
printf '%s' "$TRACEPATH_PASSWORD" | tracepath login --url https://<instance> --username you@example.com --password-stdin
```

Multiple instances or accounts coexist via profiles: `tracepath login --url ... --profile work`, then `tracepath profiles list` / `tracepath profiles use work`.
...
`/tracepath debug issue X` or `/tracepath debug <free-form bug description>`.
...
| Anything else (title, error message, type, file name) | Search: `tracepath exceptions list --since 7d --search "<text>"`; widen to `--since 30d` (and `--include-archived`) if empty |
...
```bash
tracepath exceptions list --since 7d --search "checkout" --output json \
...
```bash
tracepath exceptions show <hash>
```
...
tracepath exceptions show <hash> --output json \
...
```bash
tracepath exceptions list --since 24h --order-by lastSeen        # what is erroring (firstSeen for regressions, count for volume)
tracepath logs query --since 24h --min-severity 17               # errors and worse
tracepath logs query --since 24h --search "payment declined"     # search log bodies
tracepath logs query --since 24h --service checkout-api --min-severity 13
tracepath endpoints list --since 24h --search "checkout"         # latency p50/p95/p99 and error counts, --order-by impact|count|p95|lastSeen
```
...
```bash
tracepath exceptions show $HASH --output json | jq -r '.occurrences[0].distributedTraceId' \
  | xargs -I{} tracepath logs query --trace-id {} --output json
```
...
```bash
OCC=$(tracepath exceptions show $HASH --output json | jq -c '.occurrences[0]')
TS=$(jq -r '.recordedAt' <<<"$OCC")
DT=$(jq -r '.distributedTraceId // empty' <<<"$OCC")
SID=$(jq -r '.sessionId // empty' <<<"$OCC")
[ -n "$DT" ]  && tracepath traces show "$DT" --recorded-at "$TS"      # every endpoint/task/ai-trace/exception node across services
[ -n "$SID" ] && tracepath sessions show "$SID" --started-at "$TS"    # the session + the exceptions that fired in it
```
...
```bash
tracepath metrics query --name system.cpu.utilization --aggregation max --since 24h
tracepath metrics query --name <name> --aggregation avg|sum|count|min|max [--tag key=value] [--group-by <tag>]
```

The CLI also accepts `p50|p95|p99`, but the server has no quantile aggregation for metric points and silently computes `avg` for them — never present those as percentiles. Latency percentiles come from `tracepath endpoints list`, computed from raw request durations. There is no `metrics list`; a bogus name returns an empty `series: {}` cleanly, so probing names is safe. Host metrics from the TracePath OTel Agent live under `system.*` names, and OTLP histogram metrics are stored as two series, `<name>.avg` and `<name>.count`.
...
- If the area has no instrumentation at all, set it up first with the `tracepath-setup` skill rather than bolting on one log line.
...
```bash
tracepath logs query --trace-id <id> --output json
tracepath exceptions show <hash> --output json | jq '.occurrences | last | .attributes'
```
...
Summarize: symptom, evidence (exception hashes, log excerpts, metric anomalies), root cause, fix. Include `tracepath exceptions show <hash>` references so the user can verify. After a fix is deployed and verified, archive only when the user asks:
...
`/tracepath perf <endpoint or symptom>`, or any request about slowness, high latency, p95/p99, or "why is X slow". Approach it as a senior performance engineer: quantify first, localize the cost to a span, confirm the cause against the checklist, separate code from saturation, then root cause. Reads only; never archive or mutate.

For the full methodology and the bottleneck checklist (Select N+1, missing index, chatty IPC, connection-pool exhaustion, GC pauses, and ~20 more, each tied to the TracePath signal that confirms it), read `performance.md` in this skill directory (over MCP: the `tracepath://knowledge/performance` resource). It is the authoritative reference.

The loop, using the read commands documented in this skill:

1. **Quantify and localize.** `tracepath endpoints list --since 24h --order-by p95` (or `impact`). Read the latency shape first: p50 already high means every request pays it (systemic: query, index, algorithm); p99 much greater than p50 means a tail (contention, pool exhaustion, GC, retries, a flaky dependency). Note that `impact`/`impactReason` are offset-adjusted for marked-slow endpoints but the raw p50/p95/p99 are not.
2. **Check the accepted baseline.** Before calling an endpoint slow, run `tracepath endpoints slow "<endpoint>"`. A non-zero `offsetMs` means an operator accepted that much latency (with a `reason`), so only latency *beyond* the offset is a real regression; `offsetMs 0` means default thresholds apply. Skip this only for a clearly systemic, multi-endpoint slowdown.
3. **Pinpoint when it started: adjust the window down.** Do not investigate the default window blindly; narrow it until it brackets the onset. `tracepath endpoints chart --metric-type p95 --interval-minutes <n>` returns latency over time for the top endpoints (`{timestamp, endpoint, value}` in ms): read down one endpoint's buckets for the step where p95 jumps. Confirm the cause with `tracepath metrics query --name <metric> --interval-minutes <n>` (the infra/runtime curve). For a specific route not in the top 5, bisect `tracepath endpoints list --search <name> --from <a> --to <b>` over adjacent windows. See `performance.md`, "Pinpointing when the slowness started".
4. **Get a representative slow trace.** `tracepath endpoints show <endpointId> --recorded-at <t>` for one request's span waterfall, or `tracepath traces show <distributedTraceId> --recorded-at <t>` for the cross-service timeline. Take the trace from inside the slow window you just found; find the long pole.
...
6. **Separate code from saturation.** Pull infra/runtime metrics over the same window: `tracepath metrics query --name system.cpu.utilization --aggregation max`, then `mem.used`, `go.gc_pause`. A spike that lines up with the latency onset points at saturation, not a code bug.
...
| `tracepath exceptions list` | Grouped exceptions; `--search`, `--search-type text\|regex`, `--order-by lastSeen\|firstSeen\|count`, `--include-archived` |
| `tracepath exceptions show <hash>` | One group: full stack trace + occurrences |
| `tracepath exceptions occurrence <id> --recorded-at <t>` | One occurrence by id (fast): full detail + `sessionId` + recording |
| `tracepath exceptions archive/unarchive <hash>...` | Mutating; explicit user request + `--yes` only |
| `tracepath logs query` | Logs; `--search` (`--search-type body\|attribute`), `--service`, `--min-severity <n>`, `--trace-id` |
| `tracepath endpoints list` | Per-endpoint p50/p95/p99 and counts; `--search`, `--order-by impact\|count\|p95\|lastSeen` |
| `tracepath endpoints chart` | Latency over time for the top endpoints; `--metric-type total_time\|p50\|p95\|p99`, `--interval-minutes`. Use to find when latency changed |
| `tracepath endpoints slow <endpoint>` | Whether an operator marked this endpoint slow: `{offsetMs, reason}`. `offsetMs 0` = not marked. The offset is the accepted-latency baseline |
| `tracepath endpoints show <id> --recorded-at <t>` | One request by id: span waterfall + linked errors |
| `tracepath tasks show <id> --recorded-at <t>` | One background task run by id |
| `tracepath ai-traces show <id> --recorded-at <t>` | One AI trace by id + its conversation |
| `tracepath sessions show <id> --started-at <t>` | One session by id + the exceptions that fired in it |
| `tracepath traces show <id> --recorded-at <t>` | Distributed trace: every service node sharing the id |
| `tracepath metrics query --name <metric>` | Time series; `--aggregation`, `--tag`, `--group-by`, `--interval-minutes` |
| `tracepath profiles {list,use}`, `login`, `logout`, `version` | Profile and session management |
...
tracepath exceptions list --since 1h --order-by lastSeen --page-size 10 --output json \
  | jq '.data[]? | {hash: .exceptionHash, count, lastSeen}'

# Did anything NEW break since a deploy at 13:00 UTC
tracepath exceptions list --from 2026-06-11T13:00:00Z --to "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
...
tracepath endpoints list --since 1h --order-by p95 --page-size 1 --output json | jq '.data[0]'

# Errors for one service (exceptions --search is free text, not a service filter; use logs)
tracepath logs query --service checkout-api --min-severity 17 --since 1h --output json \
  | jq '.data[]? | {timestamp, body, traceId}'
```

Empty results (`data: null` or `data: []`) are not errors: widen the window, re-check the active project (`tracepath projects list`), and if the app was never connected to TracePath, set it up first (the `tracepath-setup` skill).
